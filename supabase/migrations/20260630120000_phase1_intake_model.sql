...
-- Adds the data foundation municipalities submit on intake and the
-- kontaktperson (professional) profile attributes needed for matching.
...
-- ---------------------------------------------------------------------------
-- LOOKUP TABLES (scalable — new options can be inserted without a migration)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- JUNCTION TABLES (case <-> lookup, many-to-many)
-- ---------------------------------------------------------------------------
...
-- lookup tables: any authenticated user may read; only admin may manage options
...
CREATE POLICY "problem_areas_write_policy"  ON public.problem_areas FOR ALL
...
CREATE POLICY "goals_lookup_write_policy"  ON public.goals_lookup FOR ALL
  USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "special_wishes_lookup_write_policy"  ON public.special_wishes_lookup FOR ALL
  USING (is_admin()) WITH CHECK (is_admin());

-- junction tables: same visibility as the parent case (admin or assigned professional); admin-only writes
...
CREATE POLICY "case_problem_areas_select_policy" ON public.case_problem_areas
  FOR SELECT USING (
    is_admin()
    OR EXISTS(
      SELECT 1 FROM public.case_assignments
      WHERE case_id = case_problem_areas.case_id AND professional_id = auth.uid() AND ended_at IS NULL
    )
  );
CREATE POLICY "case_problem_areas_write_policy" ON public.case_problem_areas FOR ALL
  USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "case_goals_select_policy" ON public.case_goals
  FOR SELECT USING (
    is_admin()
    OR EXISTS(
      SELECT 1 FROM public.case_assignments
      WHERE case_id = case_goals.case_id AND professional_id = auth.uid() AND ended_at IS NULL
    )
  );
CREATE POLICY "case_goals_write_policy" ON public.case_goals FOR ALL
  USING (is_admin()) WITH CHECK (is_admin());
...
CREATE POLICY "case_special_wishes_select_policy" ON public.case_special_wishes
  FOR SELECT USING (
    is_admin()
    OR EXISTS(
      SELECT 1 FROM public.case_assignments
      WHERE case_id = case_special_wishes.case_id AND professional_id = auth.uid() AND ended_at IS NULL
    )
  );
CREATE POLICY "case_special_wishes_write_policy" ON public.case_special_wishes FOR ALL
  USING (is_admin()) WITH CHECK (is_admin());

-- ---------------------------------------------------------------------------
-- VIEW: convenience aggregation for case detail / future matching reads
-- ---------------------------------------------------------------------------
...
INSERT INTO public.problem_areas (code, label_da, sort_order) VALUES
  ...
  ('SELF_HARM',          'Selvskade',             50),
  ('SUICIDE_RISK',       'Selvmordsrisiko',       60),
  ('EATING_DISORDER',    'Spiseforstyrrelse',     70),
  ('SCHOOL_REFUSAL',     'Skolevægring',          80),
  ('SUBSTANCE_ABUSE',    'Misbrug',               90),
  ('CRIMINALITY',        'Kriminalitet',         100),
  ('VIOLENCE',           'Vold',                 110),
  ('FAMILY_ISSUES',      'Familieproblematikker',120),
  ('LONELINESS',         'Ensomhed',             130),
  ('LOW_WELLBEING',      'Lav trivsel',          140),
  ('OTHER',              'Andet',                999)
ON CONFLICT (code) DO NOTHING;

INSERT INTO public.goals_lookup (code, label_da, sort_order) VALUES
  ('MAINTAIN_SCHOOL',    'Fastholde skole',         10),
  ('DAILY_STRUCTURE',    'Struktur i hverdagen',    20),
  ('REDUCE_CONFLICTS',   'Mindske konflikter',      30),
  ('EMPLOYMENT',         'Beskæftigelse',           40),
  ('WELLBEING',          'Trivsel',                 50),
  ('INDEPENDENCE',       'Selvstændighed',          60),
  ('RELATIONSHIPS',      'Relationer',              70),
  ('OTHER',              'Andet',                  999)
ON CONFLICT (code) DO NOTHING;

INSERT INTO public.special_wishes_lookup (code, label_da, sort_order) VALUES
  ('NO_PREFERENCE',              'Ingen præference',         10),
  ('MALE_PROFESSIONAL',          'Mand',                     20),
  ('FEMALE_PROFESSIONAL',        'Kvinde',                   30),
  ('EXPERIENCE_AUTISM',          'Erfaring med autisme',     40),
  ('EXPERIENCE_ADHD',            'Erfaring med ADHD',        50),
  ('EXPERIENCE_SUBSTANCE_ABUSE', 'Erfaring med misbrug',     60),
  ('DRIVERS_LICENSE',            'Kørekort',                 70),
  ('SAME_LANGUAGE_CULTURE',      'Samme sprog/kultur',       80),
  ('OTHER',                      'Andet',                   999)
ON CONFLICT (code) DO NOTHING;
